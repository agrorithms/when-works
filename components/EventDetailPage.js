...
// Shared owner/admin event detail view. adminPassword is only passed by the
// admin wrapper (app/admin/events/[id]) and grants event access via the
// server-checked admin override instead of ownership.
export default function EventDetailPage({
    ...
    const fetchData = useCallback(async () => {
        ...
        setDeletedResponses(payload.deletedResponses || [])
    ...
    const postManageAction = async (body) => {
        const response = await fetch(`/api/events/manage/${eventRef}`, {
            method: 'POST',
            headers: {
    ...
    const handleDeleteResponse = async (responseId) => {
        ...
        const { ok, payload } = await postManageAction({ action: 'delete_response', responseId })
        setDeleteActionLoading(false)
        setDeleteConfirmId(null)
        if (!ok) {
            setDeleteActionError(payload.error || 'Could not delete the response.')
            return
        }
        await fetchData()
    }

    const handleRestoreResponse = async (responseId) => {
        setDeleteActionLoading(true)
        setDeleteActionError('')
        const { ok, payload } = await postManageAction({ action: 'restore_response', responseId })
        setDeleteActionLoading(false)
        if (!ok) {
            setDeleteActionError(payload.error || 'Could not restore the response.')
            return
        }
        await fetchData()
    }
    ...
    return (
        <div className="container">
                                    ...
                                    <div style={{
                                        ...
                                        {deleteConfirmId === r.id ? (
                                            <span style={{ display: 'flex', gap: '0.35rem', alignItems: 'center', flexShrink: 0 }}>
                                                <span style={{ color: '#fca5a5', fontSize: '0.78rem' }}>Delete?</span>
                                                <button
                                                    type="button"
                                                    ...
                                                    style={{
                                                        ...
                                                        padding: '0.2rem 0.55rem', cursor: 'pointer', fontSize: '0.75rem'
                                                    }}
                                                >
                                                    Yes
                                                </button>
            ...
            {/* Deleted responses — restore is the owner's tool; the guest's
                next visit starts fresh. */}
            {deletedResponses.length > 0 && (
                <div style={{
                    marginTop: '2rem', background: '#1e293b',
                    border: '1px solid #334155', borderRadius: '8px', padding: '0.75rem 1rem'
                }}>
                    <button
                        type="button"
                    ...
                    >
                        ...
                        <span>Deleted responses ({deletedResponses.length})</span>
                    </button>

                    {showDeleted && (
                        <div style={{ marginTop: '0.75rem' }}>
                            {deleteActionError && (
                                <p style={{ color: '#fca5a5', fontSize: '0.85rem', marginBottom: '0.5rem' }}>
                                    {deleteActionError}
                                </p>
                            )}
                            {deletedResponses.map(r => (
                                <div key={r.id} style={{
                                    display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                                    gap: '0.5rem', padding: '0.45rem 0', borderBottom: '1px solid #334155'
                                }}>
                                    <div>
                                        ...
                                        <span style={{ color: '#64748b', fontSize: '0.78rem', marginLeft: '0.5rem' }}>
                                            deleted {r.deleted_at ? new Date(r.deleted_at).toLocaleDateString('en-US', {
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => handleRestoreResponse(r.id)}
                                        disabled={deleteActionLoading}
                                        style={{
