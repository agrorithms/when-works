...
1. **events** - Event metadata (title, description, date ranges, blocked dates, deadline, availability visibility, allow_plus_one boolean)
2. **responses** - Respondent availability (name, display_name, response_type: available/unavailable, dates array, confirmed status, includes_so boolean, google_email for signed-in linkage, response_token uuid — the guest's edit capability, generated by DB default and only ever revealed to its owner via `/api/respond/[slug]`)
...
All tables have RLS enabled with NO public policies (see `supabase/migrations/002_drop_public_policies.sql`) — the anon role can read/write nothing. All access goes through service-role API routes.
