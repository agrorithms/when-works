...
1. **participants** - First-class identity across events (see `supabase/migrations/003_participants.sql`)
   ...
   - `google_refresh_token` + `google_refresh_token_granted_at` (008) — the owner's offline Google credential, refreshed on every sign-in (`prompt=consent` mints a new one each time), exchanged by the daily cron to create calendar events for auto-scheduled group polls. Server-only, NEVER in any API payload. Unencrypted for now (see `docs/decisions.md`). While the OAuth app is in Testing status these expire after 7 days.
2. **events** - Event metadata (title, description, date ranges, blocked dates, deadline, availability visibility, allow_plus_one boolean)
   ...
   - `auto_schedule_on` (008) — the auto-scheduling generation date (summary date + 1), stamped atomically with the summary claim when the poll's schedule has auto-scheduling on. Nulling it cancels the pending generation (how every pause/disable path and terminal cron outcome expresses "don't/done"). `auto_scheduled_at` is the CAS claim ticket set right before the cron calls Google (cleared only for transient failures) — a double-fired cron can never create two calendar events.
...
5. **event_followups** - Hosting rounds (selected_date, timezone, status: draft/open/closed, calendar sync fields)
   - A CLOSED round with a `selected_date` and no invites is the "date decided" record written by the manual Google-generate route and the cron's auto-scheduler (`lib/hostingRounds.js`); `calendar_event_id`/`calendar_payload` hold the created Google event. Everything downstream (attendance, group page, pre-send resolved-check) keys off "latest round with a selected_date" regardless of status.
...
11. **group_schedules** - Automatic recurring polls, at most one per group (`unique(group_id)`, see `007_group_scheduling.sql`)
   ...
   - Auto-scheduling opt-in (008): `auto_schedule_enabled` + `auto_event_time` (TIME) + `auto_event_timezone` (IANA) + `auto_invite_scope` (`'all'|'responded'|'available'`). Only enableable for Google-mode groups whose owner has a stored `google_refresh_token`; applies only to schedule-created polls (`created_by_schedule_id`). Validation in `lib/autoSchedule.js` (`validateAutoScheduleConfig`), pick/bucket logic there too (confirmed responses only).
...
Migration state: 001–005 have all run. 005 dropped the legacy identity surfaces (`responses.google_email`, `event_ownerships.owner_user_id`/`owner_email`, the `user_profiles` table) — code must never reference them. IMPORTANT: 005 and the cleanup code deploy must land together; code that writes the dropped columns errors against the post-005 schema. 006 (groups) is additive-only and must run BEFORE the groups code deploys. 007 (hybrid cadence + group_schedules + event markers) is additive-only and must run BEFORE the scheduling code deploys. 008 (auto-scheduling: participant refresh token, schedule auto config, event stamps) is additive-only and must run BEFORE the auto-scheduling code deploys, in BOTH Supabase projects (dev/staging and prod). The `cadence_days` drop is still deferred to a future migration once the code is deployed everywhere.
