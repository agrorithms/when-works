...
import { normalizeEventTime } from './autoSchedule'
...
export function sanitizeSchedule(schedule) {
    ...
    return {
        ...
        auto_schedule_enabled: Boolean(schedule.auto_schedule_enabled),
        auto_event_time: normalizeEventTime(schedule.auto_event_time),
        auto_event_timezone: schedule.auto_event_timezone,
        auto_invite_scope: schedule.auto_invite_scope || 'available',
    }
}
...
export async function loadGroupBundle(supabaseAdmin, group) {
    ...
    const { data: eventRows, error: eventsError } = await supabaseAdmin
        .from('events')
        .select('id, title, slug, date_range_start, date_range_end, response_deadline, created_at, auto_schedule_on, auto_scheduled_at')
        .eq('group_id', group.id)
    ...
    // An unresolved poll stamped for generation but not yet claimed: the pause
    // flow asks whether to cancel it, and the schedule card shows it.
    const pendingEvent = events.find(
        (event) => event.auto_schedule_on && !event.auto_scheduled_at && !roundByEvent[event.id]
    )
    const pendingAutoSchedule = pendingEvent
        ? { eventId: pendingEvent.id, title: pendingEvent.title, scheduledFor: pendingEvent.auto_schedule_on }
        : null

    // Google-mode owners need a live refresh token before auto-scheduling can
    // be enabled; the UI uses this to gate the toggle and to nudge stale
    // connections (Testing-mode tokens die after ~7 days).
    let ownerCalendar = { connected: false, grantedAt: null }
    if (group.access_mode === 'google' && group.owner_participant_id) {
        ...
        if (owner?.google_refresh_token) {
            ownerCalendar = { connected: true, grantedAt: owner.google_refresh_token_granted_at }
        }
    }
