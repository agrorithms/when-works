...
import { getParticipantByEmail, normalizeEmail } from './participants'
...
//      match ownership.participant_id.
...
export async function resolveOwnership(supabaseAdmin, ref, session, request = null) {
    ...
    if (session?.user?.email && canBeUuid(ref)) {
        ...
        if (ownershipRows && ownershipRows.length > 0) {
            ...
            const isOwner = participant && ownership.participant_id === participant.id
