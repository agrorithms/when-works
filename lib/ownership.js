...
import { getParticipantByEmail, ensureParticipantForSession, normalizeEmail } from './participants'

// Canonical home is lib/participants.js; re-exported so existing importers
// keep working.
export { normalizeEmail } from './participants'
...
//   2. Google session: ref is an event id; the session's participant must
//      match ownership.participant_id, falling back to the legacy
//      owner_email / owner_user_id columns (claiming participant_id when a
//      legacy-keyed row is matched). Legacy fallbacks go away in the
//      post-005 cleanup PR.
...
export async function resolveOwnership(supabaseAdmin, ref, session, request = null) {
    ...
    if (session?.user?.email && canBeUuid(ref)) {
        ...
        if (ownershipRows && ownershipRows.length > 0) {
            ...
            const isOwner =
                (participant && ownership.participant_id === participant.id) ||
                (ownership.owner_email && normalizeEmail(ownership.owner_email) === email) ||
                (ownership.owner_user_id && ownership.owner_user_id === session.user.id)

            if (!isOwner) {
                return { error: 'Forbidden', status: 403 }
            }

            if (!ownership.participant_id) {
                ...
                if (claimant) {
                    await supabaseAdmin
                        ...
                        .eq('id', ownership.id)
                    ownership.participant_id = claimant.id
                }
            }
