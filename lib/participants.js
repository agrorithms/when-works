...
// Legacy google_email rows were written with the raw session email, so exact
// lookups miss mixed-case values — match with ilike instead. LIKE wildcards
// are escaped to keep exact-match semantics. Goes away with the post-005
// cleanup PR.
export function legacyEmailPattern(email) {
    const normalized = normalizeEmail(email)
    return normalized ? normalized.replace(/[\\%_]/g, (m) => `\\${m}`) : null
}
